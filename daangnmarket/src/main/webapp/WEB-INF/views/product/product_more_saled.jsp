			...
			<div class = "content">
				<div class = "content_t">
					...
					<span>${name}님의 판매 상품 보기</span>
						<div class = "more_menu">
						<a href = "http://localhost:9000/daangn/product_more.do?id=${id}&pid=${pid}&rno=${rno}"><button type = "button" class = "btn_filter">전체</button></a>
						...
						<a href = "http://localhost:9000/daangn/product_more_saled.do?id=${id}&pid=${pid}&rno=${rno}"><button type = "button" class = "btn_filter_onclick">거래완료</button></a>
					</div>
				</div>
				<c:forEach var = "userlist" items = "${ulist}">
					<c:if test = "${userlist.saled eq 'Y'}">
						<ul class = "sold">
							<c:choose>
